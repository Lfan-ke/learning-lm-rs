...
fn bytes_to_f32_vec(bytes: &[u8]) -> Vec<f32> {
    ...
    let float_slice = unsafe { std::slice::from_raw_parts(bytes.as_ptr() as *const f32, vec.capacity()) };
...
impl LLamaParams<f32> {
    pub fn from_safetensors(safetensor: &SafeTensors, config: &LlamaConfigJson) -> Self {
        ...
        let get_tensor= |name: &str| -> Tensor<f32> {
            ...
            return if res.len() == 0 {
                Tensor::<f32>::default(&vec![])  // 实际上得 panic
            } else {
                Tensor::<f32>::new(bytes_to_f32_vec(res[0].1.data()), &Vec::from(res[0].1.shape()))
            }
        };
        // println!("-------");
        // println!("{:?}", config);
